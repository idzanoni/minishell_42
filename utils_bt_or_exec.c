...
/*   Updated: 2024/07/23 14:46:40 by mgonzaga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	expand_with_command(t_minishell *s_minishell)
{
	...
	temp = ft_split(s_minishell->current_command[0], ' ');
	temp2 = s_minishell->current_command;
	lines_temp = 0;
	lines_cmd = 0;
	...
	while (s_minishell->current_command[lines_cmd] != NULL)
		lines_cmd++;
	s_minishell->current_command = malloc((lines_cmd + lines_temp)
			* sizeof(char *));
	lines_cmd = -1;
	while (temp[++lines_cmd] != NULL)
		s_minishell->current_command[lines_cmd] = temp[lines_cmd];
	lines_temp = 1;
	while (temp2[lines_temp] != NULL)
		s_minishell->current_command[lines_cmd++] = temp2[lines_temp++];
	s_minishell->current_command[lines_cmd] = NULL;
	free(temp2[0]);
	free(temp);
	free(temp2);
}
