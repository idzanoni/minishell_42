...
/*   Updated: 2024/07/22 17:05:07 by mgonzaga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_redirect(char **splited_prompt)
{
	...
	{
		if ((splited_prompt[count][0] == '<' || splited_prompt[count][0] == '>' ) && ft_strlen(splited_prompt[count]) <= 2)
		{
...
t_fds	find_redirect(t_minishell *s_minishell)
{
	...
	{
		if (s_minishell->current_command[count][0] == '>' && ft_strlen(s_minishell->current_command[count]) <= 2)
			right_redirect(s_minishell, &fd_redirect, &count);
		else if (s_minishell->current_command[count][0] == '<' && ft_strlen(s_minishell->current_command[count]) <= 2)
			...
			t_fds	*fd_redirect, int *count)
{
	...
	if (s_minishell->current_command[(*count)][1] == '<')
		...
		fd_redirect->fd_in = open(s_minishell->current_command[(*count) + 1],
				O_RDONLY);
}
			...
			t_fds	*fd_redirect, int *count)
{
	...
	if (s_minishell->current_command[(*count) + 1][0] != '\0')
	{
		if (s_minishell->current_command[(*count)][1] == '>')
	...
	else
	{
		...
		print_error(s_minishell->current_command[(*count) + 1], ": ambiguos redirect");
	}
}
